...
$fecha = explode("/", $_GET['dia'])[0];
$idCita = explode("/", $_GET['dia'])[1];
...
$query2 = "SELECT * FROM citas WHERE idCitas = :cita";
    $stmt2 = $bd->prepare($query2);
    $stmt2->bindParam(':cita', $idCita);
    $stmt2->execute();
...
if (count($result) > 0 && count($result) < 5) {
    ...
    for ($i = 0; $i < count($horas); $i++) {
        if (!in_array($horas[$i], $hora))$disponibles[] = $horarios[$i];
        if (in_array($horas[$i], $uHora))$disponibles[] = $horarios[$i];
    }
